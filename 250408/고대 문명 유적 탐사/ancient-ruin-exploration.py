...
def rotate(grid, i, j):
    ...
    for x in range(3):
        for y in range(3):
            new_grid[x + i][y + j] = grid[i + 3 - y - 1][j + x]
...
def count_and_remove(grid, flag):
    '''
    같은 종류의 유물 조각이 3개인 경우를 찾아 제거하고, 제거된 조각의 수를 반환,
    유물 1차 획득의 최댓값을 구하기 위해서는 일단 제거하지는 않는다.
    '''
    visited = [[False] * 5 for _ in range(5)]
    total = 0
    # print('grid',grid)
...
def simulate(K, grid, nums):
    results = []
    # K턴 반복
    for _ in range(K):
        # best = (유물 수, 회전 횟수, 열, 행, 회전 결과)
        best = (-1, 0, 0, 0, [])
        # 90, 180, 270도 -> 
